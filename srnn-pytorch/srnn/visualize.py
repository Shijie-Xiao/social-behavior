...
def plot_trajectories(true_trajs, pred_trajs, nodesPresent, obs_length, name, plot_directory, withBackground=False, coord_scale='human'):
    '''
    Parameters
    ==========
    ...
    for j in traj_data:
        ...
        if coord_scale == 'mouse':
            true_x = [p[0]/10.0 for p in true_traj_ped]
            true_y = [p[1]/10.0 for p in true_traj_ped]
            pred_x = [p[0]/10.0 for p in pred_traj_ped]
            pred_y = [p[1]/10.0 for p in pred_traj_ped]
        else:
...
def main():
    ...
    if not (args.noedges or args.temporal or args.temporal_spatial or args.attention):
        if args.use_mouse_data:
            args.attention = True
            print('Mouse dataset: defaulting to attention mode')
        else:
    ...
    if args.use_mouse_data:
        ...
        plot_directory = os.path.join('plot', 'mice', args.exp_tag, 'plot_attention')
    else:
        save_directory = 'save/'
    ...
    for i in range(len(results)):
        print(i)
        name = 'sequence' + str(i)
        plot_trajectories(results[i][0], results[i][1], results[i][2], results[i][3], name, plot_directory, withBackground, coord_scale)
