...
def sample_gaussian_2d(mux, muy, sx, sy, corr, nodesPresent):
    ...
    for node in range(numNodes):
        if node not in nodesPresent:
            continue
        # Convert tensor elements to Python values before creating lists
        mean = [o_mux[node].cpu().item(), o_muy[node].cpu().item()]
        sx_val = o_sx[node].cpu().item()
        sy_val = o_sy[node].cpu().item()
        corr_val = o_corr[node].cpu().item()
