...
def train(args):
    ...
    obs_len_for_path = max(args.seq_length - args.pred_length, 0)
    if args.use_mouse_data:
        exp_tag = args.exp_tag or "obs{}_pred{}_fs{}".format(obs_len_for_path, args.pred_length, args.frame_subsample)
    else:
        exp_tag = args.exp_tag or "obs{}_pred{}".format(obs_len_for_path, args.pred_length)

    if args.use_mouse_data:
        log_directory = os.path.join('log', 'mice', exp_tag, 'log_attention_single')
        save_directory = os.path.join('save', 'mice', exp_tag, 'save_attention_single')
    else:
        log_directory = os.path.join('log', str(args.leaveDataset), exp_tag, 'log_attention_single')
        save_directory = os.path.join('save', str(args.leaveDataset), exp_tag, 'save_attention_single')
